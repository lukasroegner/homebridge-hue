  ...
  for (const key in configJson) {
    ...
    switch (key.toLowerCase()) {
      case 'excludesensortypes':
        ...
        break
      case 'exposebridge':
        this.config.exposeBridge = !!value
      case 'groups':
      ...
      case 'lowbattery':
        this.config.lowBattery = toIntBetween(
          value, -1, 100, this.config.lowBattery
        )
        break
      case 'name':
        this.name = value
      ...
      case 'sensorduration':
        if (value) {
          this.config.sensorDuration = {}
          for (let sensorId in value) {
            if (value[sensorId]) {
              this.config.sensorDuration[sensorId] = {}
              for (let timeOfDay in value[sensorId]) {
                const split = timeOfDay.split(':');
                this.config.sensorDuration[sensorId][(parseInt(split[2]) + (parseInt(split[1]) * 60) + (parseInt(split[0]) * 3600)).toString()] = value[sensorId][timeOfDay]
              }
            }
          }
        }
        break
      case 'sensorenabledswitch':
        this.config.sensorEnabledSwitch = !!value
      default:
