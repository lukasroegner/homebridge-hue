...
function HueSensor (accessory, id, obj) {
  ...
  switch (this.obj.type) {
    ...
    case 'Geofence': // Undocumented
      ...
      if (this.bridge.platform.config.sensorEnabledSwitch) {
        this.switchService = new Service.Switch(this.name + ' - Enabled', this.subtype + '-switch');
  ...
  if (this.bridge.platform.config.sensorDuration) {
    const sensorDuration = this.bridge.platform.config.sensorDuration[this.id]
    if (sensorDuration) {
      setInterval(() => {
        const currentTimeOfDay = (new Date().getHours() * 3600) + (new Date().getMinutes() * 60) + (new Date().getSeconds());
        const timeOfDays = []
        for (let timeOfDayString in sensorDuration) {
          timeOfDays.push(parseInt(timeOfDayString))
        }

        let sordedTimesOfDay = timeOfDays.sort((a, b) => a < b);
        if (sordedTimesOfDay.filter(x => x <= currentTimeOfDay).length > 0) {
          sordedTimesOfDay = sordedTimesOfDay.filter(x => x <= currentTimeOfDay);
        } else {
          sordedTimesOfDay = timeOfDays.sort((a, b) => a > b);
        }

        this.setDuration(sensorDuration[sordedTimesOfDay[0].toString()], () => {
          this.service.updateCharacteristic(eve.Characteristic.Duration, sensorDuration[sordedTimesOfDay[0].toString()])
        });
      }, this.bridge.platform.config.heartrate * 1000)
    }
  }
}
...
HueSensor.prototype.checkOn = function (on) {
  ...
  if (this.hk.enabled !== hkEnabled) {
    ...
    if (this.bridge.platform.config.sensorEnabledSwitch && this.switchService) {
      this.switchService
        .updateCharacteristic(Characteristic.On, this.hk.enabled);
    }
  }
}
